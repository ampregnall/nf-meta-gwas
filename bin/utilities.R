...
#' @export
locus_extract <- function(sumstats_df,
  ...
  sumstats_df <- sumstats_df |>
    ...
    dplyr::filter(dplyr::between(position, lead_pos - locus_size / 2, lead_pos + locus_size / 2)) |>
    dplyr::collect() |>
    dplyr::mutate(locus_marker = glue::glue("{chromosome}:{lead_pos}")) |>
    dplyr::select(locus_marker, chromosome, lead_pos, gene, dplyr::everything())
